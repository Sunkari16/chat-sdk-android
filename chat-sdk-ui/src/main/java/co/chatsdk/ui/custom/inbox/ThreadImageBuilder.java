...
public class ThreadImageBuilder {
    ...
    public static Single<Uri> getImageUriForThread(final Context context, final Thread thread) {
        return Single.create(e -> {
            ...
            // If the URL is empty
            if (urls.size() == 0) {
                e.onError(new Throwable("Invalid Avathar"));
            }
            else if (urls.size() == 1) {
                ...
                combineBitmaps(context, urls).subscribe(bitmap -> {
                    ...
                    if(file != null) {
                        ...
                        e.onError(new Throwable("could_not_save_composite_thread_image_to_file"));
                    }
        ...
        return Single.create(e -> {
            ...
            Single.merge(singles).observeOn(AndroidSchedulers.mainThread())
                    .doOnComplete(() -> {
                        ...
                        if(bitmap == null) {
                            e.onError(new Throwable("thread_image_could_not_be_created"));
                        }
                        else {
                            e.onSuccess(bitmap);
                        }
    ...
    public static int defaultBitmapResId (Thread thread) {
        if (thread.typeIs(ThreadType.Private1to1)) {
            return R.drawable.ic_manch_avatar_placeholder;
        }
        else {
            return R.drawable.ic_manch_avatar_placeholder;
        }
